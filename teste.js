...
const { arrayJuros } = require('./services/totalValue')
const {juros} = require('./services/viability')
...
const arrayJuros = carros.map(getObject).map(getObject2).map(getId).forEach(i => i, jurosd)

console.log(arrayJuros);
