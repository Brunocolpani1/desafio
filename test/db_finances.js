...
class Database {
  ...
  async cadastrar(finances) {
    const dados = await this.obterArquivo();
    //workaround para simular um id
    const id = finances.id <= 2 ? finances.id : Date.now();
    const financesComId = {
      ...finances,
      id,
    };

    return await this.escreverArquivo([...dados, financesComId]);
  }
  ...
  async atualizar(id, atualizacoes) {
    ...
    if (indice === -1) {
      throw Error('finances não existe!');
    }
